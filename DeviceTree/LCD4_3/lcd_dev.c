...
static int lcd_probe(struct platform_device *pdev)
{
    ...
    /*
     * VIDCON1:
     * [4]:IVDEN   ===> 1 : Normal(不反转) ---> set 0
     * [5]:IVSYNC  ===> 1 : Inverted(反转)  --> set 1
     * [6]:IHSYNC  ===> 1 : Inverted(反转)  --> set 1
     * [7]:IVCLK   ===> 1 : Fetches video data at VCLK rising edge (下降沿触发) --> set 0
     * [10:9]:FIXVCLK  ====> 01 : VCLK running  --> set 1
     */
...
//    temp |= (2 << 16) | (2 << 8)  | (41);
    ...
    printk("+++ func: %s, line: %d", __FUNCTION__, __LINE__);
    s3c_lcd->screen_base = dma_alloc_writecombine(&pdev->dev, s3c_lcd->fix.smem_len, (dma_addr_t *)&s3c_lcd->fix.smem_start, GFP_KERNEL);
    printk("+++ func: %s, line: %d", __FUNCTION__, __LINE__);
